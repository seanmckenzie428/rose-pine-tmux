...
if [ "$variant" = "auto" ]; then
  tmux set-hook -g client-dark-theme "set-option -gq @rose_pine_current_variant dark \\; run-shell '$CURRENT_DIR/scripts/apply-theme.sh'" 2>/dev/null || true
  tmux set-hook -g client-light-theme "set-option -gq @rose_pine_current_variant light \\; run-shell '$CURRENT_DIR/scripts/apply-theme.sh'" 2>/dev/null || true
fi
